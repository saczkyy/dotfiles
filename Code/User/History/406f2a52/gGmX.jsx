...
export const Dodaj = () => {
  ...
  const createResponseObjectAndSaveInCache = (media) => {
    ...
    window.caches.open("cachename").then((cache) => {
      ...
      urlhref.current = url.href;
    });
    return urlhref.current;
  };

  const handleSubmit = (e) => {
    e.preventDefault();
    const photo = createResponseObjectAndSaveInCache(zdjecie.current);
    console.log(photo);
    dane.current = {
      zdjecie: photo,
      nazwa: nazwa,
  ...
  return (
    <div id="dodaj-content-grid" className="content-grid">
      ...
      <form onSubmit={handleSubmit} style={{ zIndex: 2 }} id="formWrapper">
        ...
        <div className="inputDiv">
          ...
          <input
            type="file"
            name="zdjecie"
            onChange={(e) => (zdjecie.current = e.target.files[0])}
          />
        </div>
        <div className="ocenkaDiv">
      ...
      <div style={{ zIndex: 2 }} className="full-width-image">
        <Label
          nazwa={nazwa}
          zdjecie={zdjecie.current ? URL.createObjectURL(zdjecie.current) : ""}
          zrodlo={zrodlo}
          procenty={procenty}
          cena={cena}
          pojemnosc={pojemnosc}
