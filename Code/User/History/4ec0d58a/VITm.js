...
function spawnFallingStar(){
    ...
    async function showStar(){
        ...
        for (let i=0; i < number_of_steps; i++){
            ...
            y = parable(x)
            console.log(prev_stars.length);
            if (prev_stars.length >= 3){
                // prev_stars = removeItemFromArray(prev_stars, prev_stars[0])
                // prev_stars[1].remove()
                console.log(prev_stars[0]);
            }
            prev_stars.push(spawnStar(x, y, true))
            await timer(7)
        }
        for (const s of prev_stars){
            s.remove()
        }
    }    
    showStar()
