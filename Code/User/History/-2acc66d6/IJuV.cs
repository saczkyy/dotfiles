...
public class balin : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        circle.Update(middle, radius, width_circle);
    }
    ...
    class Circle{
        ...
        public void Update(Vector2 mid, float radius, float width){
            ...
            LineRenderer lr = this.circle.GetComponent<LineRenderer>();
            ...
            for (int angle = 0; angle<360; angle++){
                float radians = (angle * math.PI) / 180;
                float x = math.cos(radians) * radius + mid[0];
                float y = math.sin(radians) * radius + mid[1];
                Vector2 point = new Vector2(x, y);
